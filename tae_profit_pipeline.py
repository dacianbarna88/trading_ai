...
# Non-terminal order statuses that remain eligible for a later fill attempt.
ELIGIBLE_RETRY_STATUSES = frozenset(
    {
...
PRE_EVOLUTION_DECISIONS_JSON = ROOT / "runtime_outputs/paper_decisions/paper_decisions_pre_evolution.json"
    ...
    *,
    ...
    if order:
        ...
        if status == "SKIPPED_NO_POSITION":
            return "no_position"
        if status == "SKIPPED_RETRY_COOLDOWN":
            return "retry_cooldown"
        if status == "BLOCKED_FAKE_PROFIT_RISK":
            return "fake_profit_risk"
        if status == "BLOCKED_HARD_RISK_AT_FILL":
            return "hard_risk"
        if status == "NO_CHANGE":
            exec_reason = _s(order.get("execution_reason")).lower()
            reason_l = reason
            if (
                "already_processed_same_action" in exec_reason
                or "same_action" in exec_reason
                or "already_processed_same_action" in reason_l
            ):
                return "same_action"
            return "no_change"
...
def classify_cycle_flow(
    *,
    block_bucket: str,
    status: str,
    executed: bool,
) -> str:
    """Classify one decision's cycle outcome for economic conversion semantics.

    Does not alter execution — reporting only.
    """
    status_u = _s(status).upper()
    bucket = _s(block_bucket)
    if executed or bucket == "executed" or status_u == "EXECUTED":
        return "newly_executed_fill"
    if bucket == "same_action":
        return "already_terminal_idempotent"
    if status_u in ELIGIBLE_RETRY_STATUSES or bucket in {"no_mark_price", "retry_cooldown", "no_position", "fake_profit_risk"}:
    ...
    if bucket in {"policy_skip", "no_change"}:
        return "non_executable_policy"
    if status_u in {"MISSING", ""} or status_u == "SKIPPED_SAME_ACTION":
        # Inferred missing order for actionable id — treat as idempotent audit gap, not a new order.
        if bucket == "same_action":
            return "already_terminal_idempotent"
        return "blocked_before_execution"
    return "blocked_before_execution"
...
def _conversion_metric(numerator: int, denominator: int, *, na_when_zero_den: bool = False) -> dict[str, Any]:
    if na_when_zero_den and denominator <= 0:
        return {
            "numerator": numerator,
            "denominator": 0,
            "rate": None,
            "not_applicable": True,
            "display": "N/A",
            "note": "No new executable orders this cycle — no new fill required (idempotent / already terminal).",
        }
    rate = round(numerator / max(denominator, 1), 4) if denominator > 0 else 0.0
    return {
        "numerator": numerator,
        "denominator": denominator,
        "rate": rate,
        "not_applicable": False,
        "display": f"{numerator}/{denominator} ({100 * rate:.1f}%)",
        "note": None,
    }


def _format_conversion_line(key: str, row: dict[str, Any]) -> str:
    if row.get("not_applicable") or row.get("rate") is None:
        note = row.get("note") or "N/A"
        return f"- {key}: **N/A** — {note}"
    return (
...
def build_profit_pipeline(*, write_outputs: bool = True) -> dict[str, Any]:
    ...
    # Cycle floor must be the PRE-evolution decision timestamp. Post-learning PDE
    # rewrites paper_decisions.json AFTER initial execution, so using the final
    # generated_at falsely excludes same-cycle orders (Orders=0 join defect).
    pre_doc = _load_json(PRE_EVOLUTION_DECISIONS_JSON) or {}
    cycle_floor_raw = pre_doc.get("generated_at") or decisions_doc.get("generated_at")
    cycle_ts = _parse_ts(cycle_floor_raw)
    ...
    for decision in decisions:
        ...
        order_status = (order or {}).get("status") or inferred_skip or "MISSING"
        order_executed = bool((order or {}).get("executed"))
        cycle_flow = classify_cycle_flow(
    ...
    order_rows_recorded = sum(
        1 for t in timelines if t["order"]["status"] not in {"MISSING", "skipped_same_action"}
    )
    already_terminal_idempotent = sum(
        1 for t in timelines if t.get("cycle_flow") == "already_terminal_idempotent"
    )
    eligible_retries = sum(1 for t in timelines if t.get("cycle_flow") == "eligible_retry")
    blocked_before_execution = sum(
        1 for t in timelines if t.get("cycle_flow") == "blocked_before_execution"
    )
    newly_executed_fills = sum(1 for t in timelines if t.get("cycle_flow") == "newly_executed_fill")
    non_executable_policy = sum(1 for t in timelines if t.get("cycle_flow") == "non_executable_policy")
    new_executable_orders = newly_executed_fills + eligible_retries + blocked_before_execution

    # Legacy aliases: orders_created previously counted all cycle order rows including
    # idempotent NO_CHANGE audits — keep field but prefer new_executable_orders for conversion.
    orders_created_cycle = order_rows_recorded
    orders_executed = newly_executed_fills
    orders_blocked_skipped = blocked_before_execution + eligible_retries
    ...
    conv_opp_signal = _conversion_metric(opp_with_signal, opp_count)
    conv_signal_actionable = _conversion_metric(
        sum(1 for t in timelines if t.get("signal") and t["pde_decision"]["action"] in ACTIONABLE_ACTIONS),
        signal_count,
    )
    conv_actionable_order = _conversion_metric(new_executable_orders, actionable_count)
    conv_order_execution = _conversion_metric(
        newly_executed_fills, new_executable_orders, na_when_zero_den=True
    )
    profitable_exec = sum(
        1 for t in timelines if t.get("cycle_flow") == "newly_executed_fill" and t["pnl"]["current_pnl"] > 0
    )
    conv_execution_profitable = _conversion_metric(profitable_exec, newly_executed_fills, na_when_zero_den=True)
    if newly_executed_fills == 0 and conv_execution_profitable.get("not_applicable"):
        conv_execution_profitable["note"] = "No newly executed fills this cycle."
    ...
    # Idempotent same_action is not a cycle "block" of a new fill — exclude from miss/avoid heuristics.
    blocked_avoided_loss = [
        t
        for t in timelines
        if t.get("cycle_flow") in {"blocked_before_execution", "eligible_retry"}
    ...
    cycle_flow_summary = {
        ...
        "order_rows_recorded": order_rows_recorded,
        "already_terminal_idempotent": already_terminal_idempotent,
        "eligible_retries": eligible_retries,
        "blocked_before_execution": blocked_before_execution,
        "non_executable_policy": non_executable_policy,
        "new_executable_orders": new_executable_orders,
        "newly_executed_fills": newly_executed_fills,
        "semantics_note": (
            "order→execution uses new_executable_orders only; "
            "already_terminal_idempotent (same_action NO_CHANGE) are not failed new orders."
        ),
    }
    ...
    payload: dict[str, Any] = {
        ...
        "cycle_execution_floor_at": cycle_floor_raw,
        "cycle_join_note": "order/trade join uses pre-evolution generated_at floor so post-learning PDE rewrite does not drop same-cycle orders",
        "cycle_flow": cycle_flow_summary,
        "summary": {
        ...
        "conversion_metrics": {
            "opportunity_to_signal": conv_opp_signal,
            "signal_to_actionable_decision": conv_signal_actionable,
            "actionable_decision_to_order": conv_actionable_order,
            "order_to_execution": conv_order_execution,
            "execution_to_profitable_outcome": conv_execution_profitable,
        },
...
def write_profit_pipeline_report(payload: dict[str, Any]) -> None:
    ...
    flow = payload.get("cycle_flow") or {}
    ...
    lines = [
        ...
        f"**Schema version:** {payload.get('version')}",
        "",
        "## Cycle flow (execution semantics)",
        "",
        f"- Decisions evaluated: **{flow.get('decisions_evaluated', summary.get('final_decisions'))}**",
        f"- Order rows recorded (incl. audit): **{flow.get('order_rows_recorded', summary.get('order_rows_recorded'))}**",
        f"- Already terminal / idempotent (`same_action`): **{flow.get('already_terminal_idempotent')}**",
        f"- Eligible retries: **{flow.get('eligible_retries')}**",
        f"- Blocked before execution: **{flow.get('blocked_before_execution')}**",
        f"- Non-executable policy (HOLD/SKIP etc.): **{flow.get('non_executable_policy')}**",
        f"- New executable orders: **{flow.get('new_executable_orders')}**",
        f"- Newly executed fills: **{flow.get('newly_executed_fills')}**",
        "",
        f"_{flow.get('semantics_note') or ''}_",
        "",
        "## Pipeline summary",
        "",
        ...
        f"- New executable orders: **{summary.get('new_executable_orders')}**",
        f"- Newly executed fills: **{summary.get('newly_executed_fills')}**",
        f"- Already terminal / idempotent: **{summary.get('already_terminal_idempotent')}**",
        f"- Order rows recorded (legacy `orders_created`): **{summary.get('orders_created')}**",
        f"- Orders blocked/skipped (new work only): **{summary.get('orders_blocked_skipped')}**",
    ...
    for key, row in conv.items():
        lines.append(_format_conversion_line(key, row if isinstance(row, dict) else {}))
    ...
    for row in (payload.get("timelines") or [])[:15]:
        lines.append(
            ...
            f"{row['order']['status']} ({row['order']['block_bucket']}/{row.get('cycle_flow')}) → "
...
def format_pipeline_section(payload: dict[str, Any]) -> list[str]:
    ...
    dq = payload.get("data_quality") or {}
    jc = dq.get("join_coverage") or {}
    o2e = conv.get("order_to_execution") or {}
    if o2e.get("not_applicable") or o2e.get("rate") is None:
        conv_line = (
            "Conversion order→execution: **N/A** — no new executable orders "
            "(idempotent / already terminal; no new fill required)"
        )
    else:
        conv_line = (
            f"Conversion order→execution: {o2e.get('numerator')}/{o2e.get('denominator')} "
            f"({100 * _f(o2e.get('rate')):.1f}%)"
        )
    lines = [
        ...
        f"Decisions: {flow.get('decisions_evaluated', summary.get('final_decisions'))} | "
        f"New executable: {flow.get('new_executable_orders', summary.get('new_executable_orders'))} | "
        f"Executed: {flow.get('newly_executed_fills', summary.get('newly_executed_fills'))} | "
        f"Idempotent: {flow.get('already_terminal_idempotent', summary.get('already_terminal_idempotent'))} | "
        f"Trades: {summary.get('trades_written')}",
