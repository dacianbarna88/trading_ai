"""Meta intelligence runtime — invokes existing meta modules and reads canonical artifacts."""
...
META_MODULES: tuple[MetaModuleSpec, ...] = (
    MetaModuleSpec(
        name="meta_intelligence_engine",
        script="tae_phase10_meta_intelligence_demo.py",
        artifact="tae_meta_intelligence.json",
    ),
    MetaModuleSpec(
        name="advisory_index",
        script="tae_advisory_index_demo.py",
...
def _run_script(root: Path, spec: MetaModuleSpec) -> dict[str, Any]:
...
def run_meta_modules(root: Path | str = ".") -> dict[str, Any]:
    root = Path(root)
    steps = [_run_script(root, spec) for spec in META_MODULES]
    ...
    try:
        ...
        advisory_summary = MetaContext.load(root).advisory_summary()
    except Exception:
        advisory_summary = {}
    ...
    return {
        ...
        "modules_connected": [spec.name for spec in META_MODULES],
