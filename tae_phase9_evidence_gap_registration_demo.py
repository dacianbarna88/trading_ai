#!/usr/bin/env python3
"""
TAE Phase IX Sprint IX.5B — Evidence Gap Registration Demo
...
from research_core.evidence_engine.evidence_gap_registration_report import (
    EvidenceGapRegistrationAudit,
...
def main() -> int:
    root = Path(".")
    logger.info("TAE Phase IX Sprint IX.5B — Evidence Gap Registration")
    logger.info("Safety: %s", SAFETY_BANNER)

    before = protected_mtime_snapshot(root)
    before_wired = is_evidence_gap_wired_in_registry(root)
    ...
    audit = EvidenceGapRegistrationAudit(root)
    report = audit.run(
    ...
    gap_reg = evidence_report.evidence_gap_registration or {}
    logger.info("Registry gap status: %s", gap_reg.get("evidence_gap_status"))
    logger.info("Reports:")
    ...
    if report.verdict.endswith("FAILED_PROTECTED_FILE_MODIFIED"):
        return 1
    if report.verdict == "EVIDENCE_GAP_REGISTRATION_INCOMPLETE":
        return 1
    return 0
