#!/usr/bin/env python3
"""
TAE Phase IX Sprint IX.2B — Evidence System Integration Demo
...
from research_core.evidence_engine.evidence_integration_report import EvidenceIntegrationAudit
...
def main() -> int:
    root = Path(".")
    logger.info("TAE Phase IX Sprint IX.2B — Evidence System Integration")
    logger.info("Safety: %s", SAFETY_BANNER)
    logger.info("Canonical registry: %s", CANONICAL_REGISTRY_MODULE)

    before = _protected_mtime_snapshot(root)
    audit = EvidenceIntegrationAudit()
    dep_map, report = audit.run()
    paths = audit.persist_all(dep_map, report)
    _verify_protected_unchanged(root, before)

    print()
    print("===== TAE EVIDENCE SYSTEM INTEGRATION — SPRINT IX.2B =====")
    print()
    print(f"Safety: {SAFETY_BANNER}")
    print(f"Canonical registry: {CANONICAL_REGISTRY_MODULE}")
    ...
    print()
    print(f"Registry count: {dep_map.registry_count} (required: 1)")
    print(f"Single registry verified: {report.single_registry_verified}")
    print(f"Integration gate verified: {report.integration_gate_verified}")
    print(f"No duplicate engine: {report.no_duplicate_engine_verified}")
    print()
    print("Integration targets:")
    for target in report.integration_targets:
        print(f"  [{target.status.value}] {target.module_path} ({target.role})")
    print()
    ...
    return 0 if report.verdict == "EVIDENCE_INTEGRATION_READY" else 1
