...
OUTPUT_JSON = generated_report("tae_profit_protection_validation.json")
OUTPUT_MD = generated_report("tae_profit_protection_validation.md")
...
def aggregate_tickers(df: pd.DataFrame, strategy_stats: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for ticker, group in df.groupby("ticker"):
        ...
        hold_value = round(float(group["hold_pnl"].sum()), 2) if "hold_pnl" in group.columns else 0.0
        rows.append(
            {
                "ticker": ticker,
                "observations": obs,
