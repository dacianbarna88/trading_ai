...
# Canonical PAPER refresh chain. Operational SSOT producers run before consumers.
# live_bot does not own accounting/GII/PPG/APPE — FPC (and structural governance) do.
# morning-audit is last so freshness reflects this cycle's producers.
CYCLE_STEPS: list[tuple[str, list[str]]] = [
    ("health", [sys.executable, "tae.py", "health"]),
    ("infrastructure_health", [sys.executable, "tae_infrastructure_health.py"]),
    ("accounting_snapshot", [sys.executable, "tae_accounting_snapshot.py"]),
    ("protect", [sys.executable, "tae.py", "protect"]),
    ("portfolio_protect", [sys.executable, "tae.py", "portfolio-protect"]),
    ("policy", [sys.executable, "tae.py", "policy"]),
    ("growth_intelligence", [sys.executable, "tae.py", "growth-intelligence"]),
...
def run_pre_pde_feedback(root: Path, step_results: list[dict[str, Any]]) -> None:
    """Delegate to canonical learning runtime (single owner of learning SSOT updates)."""
    ...
    print(
        "\n>>> [pre_pde_feedback] canonical learning cycle "
        "(longitudinal + adaptive weights + rule survival)",
        flush=True,
    )
    from tae_canonical_learning_runtime import run_canonical_learning_cycle

    cycle_t0 = trace_start("pre_pde_feedback/canonical_learning_cycle")
    # force=True: full-paper-cycle always refreshes from current artifacts under lock;
    # content-level fingerprint still skips no-op rewrites when unchanged.
    cycle = run_canonical_learning_cycle(
        project_root=root,
        source="pre_pde_feedback",
        write_reports=True,
        force=True,
        blocking_lock=True,
    )
    trace_end("pre_pde_feedback/canonical_learning_cycle", cycle_t0)
    steps = cycle.get("steps") or {}
    mem = steps.get("longitudinal") or {}
    weights = steps.get("adaptive_weights") or {}
    survival = steps.get("rule_survival") or {}
    step_results.append(
        {
            "step": "pre_pde_longitudinal_memory",
            "ok": bool(mem.get("ok", cycle.get("ok"))),
            "exit_code": 0 if cycle.get("ok") else 1,
            "total_records": mem.get("total_records") or cycle.get("outcomes_evaluated"),
        }
    )
    step_results.append(
        {
            "step": "pre_pde_adaptive_weights",
            "ok": bool(weights.get("ok", cycle.get("ok"))),
            "exit_code": 0 if cycle.get("ok") else 1,
            "actions_weighted": None,
            "canonical_cycle_id": cycle.get("cycle_id"),
            "canonical_result": cycle.get("result"),
        }
    )
    step_results.append(
        {
            "step": "pre_pde_rule_survival",
            "ok": bool(survival.get("ok", cycle.get("ok"))),
            "exit_code": 0 if cycle.get("ok") else 1,
            "rules_classified": None,
            "learning_updates_applied": cycle.get("learning_updates_applied"),
            "duplicates_skipped": cycle.get("duplicates_skipped"),
        }
    )
    print(
        f">>> [pre_pde_feedback] cycle_id={cycle.get('cycle_id')} "
        f"result={cycle.get('result')} updates={cycle.get('learning_updates_applied')}",
        flush=True,
    )
    trace_end("pre_pde_feedback", t0)
...
def collect_summary(
    ...
    *,
    ...
    return {
        ...
        "dpe_winner": (evaluation.get("overall") or {}).get("winner") or evaluation.get("preferred_philosophy"),
